...
function validate_neumann_boundary_conditions(params)
    for species in params.species
        if species < 1
            throw(ArgumentError("The species $species is not a valid species. Indicies must be positive and taken from the SPECIES_VECTOR in config.jl"))
        end
...
function apply_neumann_for_initial_timestep!(sys, parameters)
    neumann_parameters = parameters.boundary_conditions.neumann
    # we only want the boundary conditions for the PHI_EQ
    # so we first check if there are any boundary conditions
    if PHI_EQ ∈ neumann_parameters.species
        # know we need to apply the correct boundary conditions,
        # so we loop over the boundary_indices and boundary values
        for (id, value) in zip(neumann_parameters.boundary_indices[PHI_EQ], neumann_parameters.boundary_values[PHI_EQ])
            boundary_neumann!(sys, PHI_EQ, id, value)
        end
    end
end
...
function apply_neumann_for_time_dependent!(sys, parameters)
    neumann_parameters = parameters.boundary_conditions.neumann
    # we loop over all the different species that we have
    for species in neumann_parameters.species
        # know we need to apply the correct boundary conditions,
        # so we loop over the boundary_indices and boundary values
        println("Boundary conditions")

        for (id, value) in zip(neumann_parameters.boundary_indices[species], neumann_parameters.boundary_values[species])
            print("Species " * string(species))
            print("ID" * string(id))
            print("value " * string(value))
            println("")
            boundary_neumann!(sys, species, id, value)
        end
    end
end
...
@option struct BoundaryConditions
    ...
    neumann::NeumannBoundary = NeumannBoundary()
end
